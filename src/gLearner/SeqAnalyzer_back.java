...
public class SeqAnalyzer_back {
        ...
        try {
            ...
            while ((line = br.readLine()) != null && line.length() > 0) {
                ...
                for(String s: labels){
                    if(s.isEmpty())
                        continue;
                    seq.assignLabel(tokenIdx++, s, getLabelIndex(s));//dynamically expand labelNames
                }
    ...
    public String4Learning getStr4Learning(Sequence seq, String mode, Map<Integer, Double> weights){
        ...
        node_features = constructNodeFeature(seq.getTokenIDs());
        ...
        for(int j = 0; j < varNodeSize; j++) {// for each node/variable
            ...
            for (Integer type : node_features.keySet()) {// for each feature type
                ...
                if(mode.equals("train")){
                    ...
                    } else if (cur_token == 1) {// is digit: type=1,3,5,7,9
                        feature_value_arr[cur_label] = cur_token;
                        ptl = LogTableFactor.makeFromValues(new Variable[]{allVars[j]}, feature_value_arr);
                        factorList.add(ptl);
        ...
        position_features = constructPositionFeature(seq.getTokenIDs());
            ...
            for (Integer type : position_features.keySet()) {// for each feature type
                ...
                if (mode.equals("train")) {
                    if (cur_token == 1) {
                        ...
                        ptl = LogTableFactor.makeFromValues(new Variable[]{allVars[j]}, feature_value_arr);
                        factorList.add(ptl);
                        featureType.add(cur_feature_size + 10 + m_labelNames.size() * (type - 11)
                                + cur_label);
                    }
